    ...
    NavHost(
        ...
        composable(route = Rutas.HOME) {
            HomeScreen(
                ...
                // --- NUEVO: Conexión con la pantalla de API ---
                onJuegosClick = {
                    navController.navigate("juegos_api")
                }
            )
        }
        ...
        // Ruta 8: Pantalla de Juegos (API Externa)
        composable(route = "juegos_api") {
            // Instanciamos el ViewModel aquí mismo
            val juegosViewModel: com.example.labx.ui.viewmodel.JuegosViewModel = androidx.lifecycle.viewmodel.compose.viewModel()

            com.example.labx.ui.screen.JuegosGratisScreen(
